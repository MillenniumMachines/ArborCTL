...
if { global.arborState[param.S][0] == null }
    ...
    M2601 E0 P{param.C} A{param.A} F3 R{var.motorLimitsAddr} B3
    var motorCfg = { global.arborRetVal }

    ; 0 = Max Frequency, 1 = Min Frequency
    M2601 E0 P{param.C} A{param.A} F3 R{var.maximumFrequencyAddr} B1
    var maxFrequency = { global.arborRetVal }
    M2601 E0 P{param.C} A{param.A} F3 R{var.minimumFrequencyAddr} B1
...
M2601 E0 P{param.C} A{param.A} F3 R{var.stateBytesAddr} B9
...
if { (var.vfdRunning && !var.vfdForward && !var.vfdReverse) || (!var.vfdRunning && (var.vfdForward || var.vfdReverse)) }
    M2600 E0 P{param.C} A{param.A} F6 R{var.stateBytesAddr} B{10,}
...
if { !var.shouldRun && var.vfdRunning }
    ...
    M2600 E0 P{param.C} A{param.A} F6 R{var.setCommandAddr} B{0x0001,}
    ; Set frequency to 0
    M2600 E0 P{param.C} A{param.A} F6 R{var.setFrequencyAddr} B{0x0000,}
    ...
    if { spindles[param.S].state == "forward" && (!var.vfdRunning || !var.vfdForward) }
        M2600 E0 P{param.C} A{param.A} F6 R{var.setCommandAddr} B{0x0012,}
        set var.commandChange = true
