...
; Huanyang clones can be slow between request/response frames.
; Use the same conservative pacing as config/probe flows.
var cmdWait = 250
...
if { global.arborState[param.S][3] == null }
    ...
    var haveWizardMotor = { exists(global.arborMotorSpec) && global.arborMotorSpec[param.S] != null && exists(global.arborWizardFreqLimits) && global.arborWizardFreqLimits[param.S] != null }
    ...
    if { !var.haveWizardMotor }
        ...
        var fallbackPoles = 2
        var fallbackMinFreq = 0
        var fallbackMaxFreq = 400
        var motorCfg = { vector(6, 0) }
        set var.motorCfg[0] = 1.5
        set var.motorCfg[1] = var.fallbackPoles
        set var.motorCfg[2] = 110
        set var.motorCfg[3] = 400
        set var.motorCfg[4] = 7.0
        set var.motorCfg[5] = 24000
        var freqConv = { vector(1, 1.0) }
        set global.arborState[param.S][0] = { var.motorCfg, var.freqConv, 0 }
...
var badSetFreq = { var.rawSetFreq == null || #var.rawSetFreq < 5 }
...
if { var.haveStatusFrame }
    set var.setFreq = { (var.rawSetFreq[3] * 256 + var.rawSetFreq[4]) / 100.0 }
    set var.currentFreq = { (var.rawOutFreq[3] * 256 + var.rawOutFreq[4]) / 100.0 }
    set var.outputCurrent = { (var.rawCurrent[3] * 256 + var.rawCurrent[4]) / 10.0 }
    set var.vfdRunning = { var.currentFreq > 0.1 || var.setFreq > 0.1 }
...
if { var.haveStatusFrame && !var.vfdRunning }
