...
; *** WRITE THE USER VARS FILE FIRST ***
; This must happen before any VFD communication so that even if M260.4
; throws an unrecoverable error, the user vars file exists and arborctl.g
; can load on the next boot.
...
echo >>{var.wizUVF} {"set global.arborMotorSpec[" ^ var.wizSpdlID ^ "] = {" ^ var.wizMotorW ^ ", " ^ var.wizMotorU ^ ", " ^ var.wizMotorV ^ ", " ^ var.wizMotorF ^ ", " ^ var.wizMotorI ^ ", " ^ var.wizMotorR ^ "} ; Wizard motor (kW,poles,V,Hz,A,RPM)"}
...
M291 P{var.cfgPrompt} R"ArborCtl: Configuration Wizard" S4 T0 K{"Yes","No"} F0 J2
if { result == -1 }
    abort { "ArborCtl: Operator aborted configuration wizard!" }
...
M291 P{"Configuration saved! Reboot now to apply settings."} R"ArborCtl: Configuration Wizard" S3 T0
M999
